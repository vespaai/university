...
def perform_cross_validation(
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y), 1):
        ...
        mae = mean_absolute_error(y_val, oof_pred[val_idx])

        fold_rmses.append(rmse)
        fold_maes.append(mae)

        logging.info(f"Fold {fold}: RMSE = {rmse:.4f}, MAE = {mae:.4f}")
    ...
    print("-" * 60)
    print(
        f"{'RMSE':<18} | {np.mean(fold_rmses):<18.4f} | {np.std(fold_rmses):<18.4f}"
    )
    print(f"{'MAE':<18} | {np.mean(fold_maes):<18.4f} | {np.std(fold_maes):<18.4f}")
    print("-" * 60)
    print(f"Overall CV RMSE: {overall_rmse:.4f} • MAE: {overall_mae:.4f}")
    print("-" * 60)
    ...
    if imp_mean is not None:
        ...
        try:
            ...
            # Save native LightGBM format for Python predictions
            native_model_file = str(out_path).replace('.json', '.txt')
            final_model.save_model(native_model_file)
            logging.info(f"Native model saved to {native_model_file}")
        except Exception as e:
