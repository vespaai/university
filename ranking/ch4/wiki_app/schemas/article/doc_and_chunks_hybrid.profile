rank-profile doc_and_chunks_hybrid inherits per_doc, per_chunk_lexical, per_chunk_semantic {
    ...
    function chunk_hybrid_scores() {
        ...
        expression {
            merge(
                chunk_sim_scores(), normalized_chunk_bm25_scores(8),
                f(sim, text)(sim + text)
            )
        }
    }
    ...
    function top_3_chunk_hybrid_scores() {
        expression: top(3, chunk_hybrid_scores())
    }
    ...
    first-phase {
        ...
        # we could also use a lightgbm model to generate a score from these, like in ch3
        expression {
            native_rank_chunk_ts() +
